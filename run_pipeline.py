...
PDF = "data/Statistical_Nepalese_Agriculture-9-45.pdf"
RAW_DB = "data/raw_tables.db"
FACT_DB = "data/fact.db"
...
def main():
    ...
    if args.reextract or not os.path.exists(RAW_DB):
        results["extract"], _ = run("STAGE 1: extract PDF -> raw_tables.db",
                                    f"python ingestion/extract_to_raw.py {PDF}")
    else:
    ...
    results["normalize"], norm_out = run(
        ...
        f"python normalization/normalize_generic.py --raw-db {RAW_DB} --fact-db {FACT_DB}")

    # Stage 3 -- validate
    results["validate"], val_out = run(
        "STAGE 3: validate fact_generic",
        f"python tests/validate_summary.py --db {FACT_DB}", capture_tail=6)

    # Stage 4 -- vocabulary
    results["vocabulary"], vocab_out = run(
        "STAGE 4: vocabulary",
        f"python tests/test_vocabulary.py --db {FACT_DB}", capture_tail=6)
    ...
    if args.skip_intent:
        ...
        results["intent"], _ = run(
            ...
            f'python tests/test_intent.py --db {FACT_DB} -q "which province produced the most paddy in 2080/81"')
